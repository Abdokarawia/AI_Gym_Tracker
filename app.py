"""
AI Gym Tracker — Streamlit Cloud (No WebRTC)
============================================
Uses a proper declare_component() webcam bridge.
Streamlit.setComponentValue works correctly inside declare_component.
It does NOT work inside st.components.v1.html / st.iframe.
...
# ── Register the webcam custom component ─────────────────────────────────────
# declare_component serves index.html and injects the Streamlit JS bridge,
# making window.Streamlit + setComponentValue available inside the iframe.
_COMPONENT_DIR = Path(__file__).parent / "webcam_component"
_webcam_raw = components.declare_component("webcam_component", path=str(_COMPONENT_DIR))

def webcam_component(annotated: str = "", key: str = "webcam"):
    """
    Renders the webcam iframe.
    annotated : base64 JPEG data-URI to push to JS canvas (Python → JS)
    Returns   : base64 JPEG captured from browser (JS → Python), or None
    """
    return _webcam_raw(annotated=annotated, key=key, default=None)
...
[data-testid="stSidebar"] h2,[data-testid="stSidebar"] h3{
  color:#39ff7e!important;font-family:'Bebas Neue',sans-serif;letter-spacing:2px;}
[data-testid="stSidebar"] hr{border-color:#1e2229!important;}
[data-testid="stSidebar"] [data-baseweb="select"]>div{
  background:#1a1d25!important;border-color:#2a2f3a!important;color:#e8eaf0!important;}
[data-testid="stSidebar"] .stButton>button{
  background:#1a1d25!important;color:#e8eaf0!important;
  border:1px solid #2a2f3a!important;border-radius:8px!important;}
[data-testid="stSidebar"] .stButton>button:hover{background:#22262f!important;border-color:#39ff7e!important;}
h1{color:#39ff7e!important;font-family:'Bebas Neue',sans-serif!important;
   letter-spacing:3px!important;font-size:2.4rem!important;}
.metric-card{background:#13161c;border-radius:12px;padding:16px 20px;
  border:1px solid #1e2229;text-align:center;margin-bottom:10px;
  position:relative;overflow:hidden;}
.metric-card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;
  background:linear-gradient(90deg,#39ff7e,#00d4ff);}
.metric-value{font-size:3rem;font-weight:800;color:#39ff7e;line-height:1.1;
...
.feedback-box{background:#13161c;border-left:3px solid #39ff7e;border-radius:8px;
  padding:10px 16px;margin-top:10px;font-size:.95rem;color:#e8eaf0;}
.wrong-ex-banner{background:linear-gradient(135deg,#2d1a00,#3d2200);
  border:1px solid #f59e0b;border-radius:10px;
  padding:12px 18px;margin:8px 0;font-size:.92rem;color:#fcd34d;}
.history-log{max-height:320px;overflow-y:auto;padding-right:4px;}
.log-entry{display:flex;gap:10px;align-items:flex-start;padding:8px 10px;
  margin-bottom:5px;border-radius:8px;background:#13161c;
  border:1px solid #1e2229;font-size:.82rem;line-height:1.4;}
.log-badge{flex-shrink:0;padding:2px 7px;border-radius:4px;
  font-size:.7rem;font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-top:1px;}
...
class SquatAnalyzer:
    ...
    def analyze(self,lms,w,h):
        ...
        fb=('Go lower!' if ang>110 and self.rc.stage!='down'
            else 'Good depth! 🔥' if ang<90 else 'Stand tall!')
...
class PushUpAnalyzer:
    ...
    def analyze(self,lms,w,h):
        ...
        fb=('Lower chest!' if ang>130 and self.rc.stage!='down'
            else 'Good depth! 🔥' if ang<90 else 'Push up!')
        return dict(angle=ang,stage=self.rc.stage,count=self.rc.count,
                    feedback=fb,form_score=self.rc.angle_score())
...
class PullUpAnalyzer:
    ...
    def analyze(self,lms,w,h):
        ...
        fb=('Pull higher!' if ang>70 and self.rc.stage=='up'
            else 'Chin over bar! 🔥' if ang<50 else 'Lower slowly!')
        return dict(angle=ang,stage=self.rc.stage,count=self.rc.count,
                    feedback=fb,form_score=self.rc.angle_score())
...
class RussianTwistAnalyzer:
    ...
    def analyze(self,lms,w,h):
        ...
        if rot>30 and side!=self._ls:
            ...
            if self._t%2==0:
                self.rc.count+=1
                self.rc._angle_scores.append(_angle_score(self._tm,80,0))
                self._tm=0
...
def draw_wrong_ex_overlay(frame,suspected):
    ...
    cv2.rectangle(ov,(0,0),(W,48),(10,90,200),-1)
...
def draw_hud(frame,res,ex_title):
    ...
    stage=(res.get('stage') or 'READY').upper()
...
class EventHistory:
    ...
    def add(self,kind,message):
        with self._lock:
            self._events.appendleft({'time':datetime.now().strftime('%H:%M:%S'),
...
# ── Gym state ─────────────────────────────────────────────────────────────────
_GYM_STATE_VERSION = 5   # bump whenever GymState gains/removes attributes
...
class GymState:
    ...
    def process_b64_frame(self, b64_data: str) -> str:
        """Decode browser frame → MediaPipe → draw HUD → return annotated b64."""
        try:
            _, enc = b64_data.split(',', 1) if ',' in b64_data else ('', b64_data)
            frame  = cv2.imdecode(np.frombuffer(base64.b64decode(enc), np.uint8),
                                  cv2.IMREAD_COLOR)
        ...
        with self.lock:
            ...
            if det.pose_landmarks:
                ...
                if is_w:
                    ...
                    self.history.add('wrong',
                        f'Detected {sus} (conf {conf:.0%}) — selected: {self.exercise}')
            ...
            self._fps_frames += 1
            now = time.time()
            if now - self._fps_last >= 2.0:
                self.current_fps   = self._fps_frames / (now - self._fps_last)
                self._fps_frames   = 0
                self._fps_last     = now
...
def _is_fresh(o):
    return isinstance(o, GymState) and getattr(o, '_version', 0) == _GYM_STATE_VERSION
...
st.caption("MediaPipe Pose · Custom Component Webcam · No WebRTC · Streamlit Cloud ✅")
...
def render_stats():
    ...
    if gym.wrong_ex_flag:
        wrong_ex_ph.markdown(
            f'<div class="wrong-ex-banner">⚠️ Wrong exercise!<br>'
    ...
    fps_ph.markdown(
        f'<div class="metric-card">'
        f'<div class="metric-value" style="font-size:1.6rem;color:#f59e0b;">'
        f'{gym.current_fps:.1f}</div>'
...
if mode.startswith("📹"):
    with col_vid:
        ...
        # Pull previously annotated frame (set on last rerun)
        annotated_b64 = st.session_state.get('annotated_frame', '')

        # Render custom component:
        #   Python  →  annotated=annotated_b64  →  JS draws on canvas
        #   JS      →  Streamlit.setComponentValue(raw_jpeg)  →  Python receives
        raw_frame = webcam_component(
            annotated=annotated_b64,
            key=f"webcam_{exercise}",
        )
            ...
            if st.button("🧠 Analyze Video", type="primary", use_container_width=True):
                ...
                while True:
                    ...
                    if det.pose_landmarks:
                        ...
                        if is_w:
                            draw_wrong_ex_overlay(frame, sus)
                            gym.history.add('wrong', f'[Frame {fidx}] {sus} ({conf:.0%})')
